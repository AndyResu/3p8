		...
		for spell=1,#self.spellList do
			if !castDone then
				...
				for i=1,#self.plyUniquePos do
					...
					if !(self:similarDistance(relativePos,relativePosSpell,self.toleranceDistance)) then
						isGoodToCast = false
						print("here is bad")
...
function ENT:Initialize()
	...
	if self.devMode && (#self.spellDerivativeList < 1 || #self.spellDist < 1 || #self.spellPercent < 1) then
		self.spellDerivativeList = {}
		self.spellDist = {}
		self.spellPercent = {}
		for spell=1,#self.spellList do
			...
			for i=1,#self.spellList[spell]-1 do
				...
				-- table.insert(self.spellPythag[spell], pythagHere)
