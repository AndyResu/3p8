...
function ENT:AddItem(entName, amount)
	...
	--if (!isFound) then
		items[#items+1].ent = entName
		items[#items+1].stock = amount
	--end
end
...
function ENT:RemoveItem(entName, amount)
	...
	for i=1, #items do
		...
		if(isValid(items[i]) && items[i].ent == entName && items[i].stock >= amount) then
			items[i].stock = items[i].stock - amount
...
--for arrival to a place (handled by the place currently)
